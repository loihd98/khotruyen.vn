...
const AdminSidebar: React.FC<AdminSidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 lg:shadow-none ${
                  ...
                  {group.items.map((item) => (
                    ...
                    >
                      <span className={`w-1.5 h-1.5 rounded-full mr-3 ${
                        activeTab === item.id ? "bg-blue-400" : "bg-gray-600"
                      }`}></span>
                      {item.label}
                    </button>
                  ))}
